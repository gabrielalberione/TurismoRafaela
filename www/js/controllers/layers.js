...
function lc_crear_tabla(){
	...
	}, lc_errorCB, lc_successCB);	
}

function lc_errorCB(tx, err){
	console.log("error en trasaccion de layer: "+err);
}
...
function lc_get_layers(pGrouplayer_id, func_success){
	...
	dbGuiaR.transaction(function(tx) {
		tx.executeSql('SELECT * FROM LAYERS WHERE grouplayer_id = ?', [pGrouplayer_id], func_success, lc_errorCB);
	}, lc_errorCB, lc_successCB);
}